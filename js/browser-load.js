	...
	init(callback) {
		...
		try {
			...
			if (primus_react_client_wallet.initialized === false) {
				console.log('WARNING: @primusmoney/react_client_wallet should be initialized before initializing @primusmoney/react_client_wallet');
			}
			...
			// web module ready
			rootscriptloader.registerEventListener('@primusmoney/on_primus_react_pwa_module_ready', function(eventname) {
			...
			xtrawebscriptloader.load_scripts(function() {
				rootscriptloader.signalEvent('@primusmoney/on_primus_react_pwa_module_ready');
			});
